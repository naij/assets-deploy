...
class GitWorker extends BaseWorker {
  ...
  * cloneCode() {
    ...
    yield fse.remove(this.assetsWorkspace)

    try {
      result = yield exec(`git clone ${this.config.gitSshUrl}:${this.owner}/${this.repo}.git --depth=1 -b ${this.refName} ${this.assetsWorkspace} > /dev/null 2>&1`)
    } catch(e) {
